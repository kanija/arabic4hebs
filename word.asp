...
if not (session("role") And 2)>0 then %>
  ...
  border-width:1px 1px 1px 0; margin-bottom:5px;">על מנת לערוך, עליך <a href="login.asp">להתחבר</a></span><%
end if %>
    <div>
    ...
    <div>
        ...
        <script>
            function exReset() {
                ...
                if (explain != "") location.href='<%=baseT%>word.reset.asp?wordID=<%=wordId%>&status=<%=status%>&explain='+explain;
...
<div id="reportErr" class="table team">
    ...
    <form action="<%=baseT%>word.erroneous.asp">
